...
/* ----------------- OUTPUT ----------------- */
  outFastK = mxCalloc(inSeriesRows, sizeof(int));
  outFastD = mxCalloc(inSeriesRows, sizeof(int));
/* -------------- Invocation ---------------- */
...
/* -------------- Errors ---------------- */
  ...
  memcpy(mxGetData(plhs[0]), outFastK, (outBegIdx+outNbElement)*mxGetElementSize(plhs[0]));
  mxFree(outFastK);  
  plhs[1] = mxCreateNumericMatrix(outBegIdx+outNbElement,1, mxINT32_CLASS, mxREAL);
  memcpy(mxGetData(plhs[1]), outFastD, (outBegIdx+outNbElement)*mxGetElementSize(plhs[1]));
  mxFree(outFastD);  
} /* END mexFunction */
