...
/* ----------------- OUTPUT ----------------- */
  outMin = mxCalloc(inSeriesRows, sizeof(double));
  outMax = mxCalloc(inSeriesRows, sizeof(double));
/* -------------- Invocation ---------------- */
...
/* -------------- Errors ---------------- */
  ...
  memcpy(((double *) mxGetData(plhs[0]))+outBegIdx, outMin, outNbElement*mxGetElementSize(plhs[0]));
  mxFree(outMin);  
  plhs[1] = mxCreateDoubleMatrix(outBegIdx+outNbElement,1, mxREAL);
  memcpy(((double *) mxGetData(plhs[1]))+outBegIdx, outMax, outNbElement*mxGetElementSize(plhs[1]));
  mxFree(outMax);  
} /* END mexFunction */
