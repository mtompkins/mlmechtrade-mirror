...
/* ----------------- OUTPUT ----------------- */
  outAroonDown = mxCalloc(inSeriesRows, sizeof(double));
  outAroonUp = mxCalloc(inSeriesRows, sizeof(double));
/* -------------- Invocation ---------------- */
...
/* -------------- Errors ---------------- */
  ...
  memcpy(((double *) mxGetData(plhs[0]))+outBegIdx, outAroonDown, outNbElement*mxGetElementSize(plhs[0]));
  mxFree(outAroonDown);  
  plhs[1] = mxCreateDoubleMatrix(outBegIdx+outNbElement,1, mxREAL);
  memcpy(((double *) mxGetData(plhs[1]))+outBegIdx, outAroonUp, outNbElement*mxGetElementSize(plhs[1]));
  mxFree(outAroonUp);  
} /* END mexFunction */
