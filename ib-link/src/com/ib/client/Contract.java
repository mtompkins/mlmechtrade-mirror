...
public class Contract implements Cloneable {
	@Override
	public int hashCode() {
		return (m_symbol + m_secType + m_secType).hashCode();
	}
	...
	public String m_primaryExch; // pick an actual (ie non-aggregate)

	// exchange that the contract trades on. DO
	// NOT SET TO SMART.
	...
	public Contract(String p_symbol, String p_secType, String p_expiry,
			...
			boolean p_includeExpired, String p_cusip, String p_ratings,
			String p_descAppend, String p_bondType, String p_couponType,
			boolean p_callable, boolean p_putable, double p_coupon,
			boolean p_convertible, String p_maturity, String p_issueDate,
			String p_nextOptionDate, String p_nextOptionType,
			boolean p_nextOptionPartial, String p_notes) {
		m_symbol = p_symbol;
		m_secType = p_secType;
		m_expiry = p_expiry;
		m_strike = p_strike;
	...
	public boolean equals(Object p_other) {
		if (p_other == null
				...
				|| m_comboLegs.size() != ((Contract) p_other).m_comboLegs
						.size()) {
			return false;
		...
		if (!l_thisSecType.equals(l_otherSecType)) {
			...
			l_bContractEquals = l_thisSymbol.equals(l_otherSymbol)
				...
				if (l_thisSecType.equals("BOND")) {
					...
					if (l_bContractEquals) {
						...
						String l_otherNotes = l_theOther.m_notes != null ? l_theOther.m_notes
								: "";
						l_bContractEquals = l_thisCusip.equals(l_otherCusip)
					...
					l_bContractEquals = l_thisExpiry.equals(l_otherExpiry)
		...
		if (l_bContractEquals && m_comboLegs.size() > 0) {
			...
			for (int ctr1 = 0; ctr1 < m_comboLegs.size(); ctr1++) {
				...
				if (!l_bLegsEqual) { // leg on first not matched by any
					// previously unmatched leg on second
					return false;
				}
			}
		}
